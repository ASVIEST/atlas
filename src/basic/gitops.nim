...
proc fetchRemoteHeadsByName(path: Path; remote: string; errorReportLevel: MsgKind = Warning): bool =
  ...
  let (outp, status) = exec(GitFetchHeads, path, args, errorReportLevel)
  if status != RES_OK:
...
proc tipFromRef(path: Path; remoteName, remoteRef: string; errorReportLevel: MsgKind; isLocalOnly: bool): VersionTag =
  ...
  var args: seq[string] = @["-n1"]
  if cmd == GitLog:
    args.add remoteRef
  let (outp1, status1) = exec(cmd, path, args, Warning)
...
proc pushTag*(path: Path; origin = "origin"; tag: string) =
  ...
  if remote.len == 0:
    ...
    return
  let (outp, status) = exec(GitPush, path, [remote, tag])
  if status != RES_OK:
